...
environment.etc."sway/config.d/extras.conf".source = let
...
configure-qt = pkgs.writeTextFile {
  name = "configure-qt";
  destination = "/bin/configure-qt";
  executable = true;
  text = let
  in ''
...
in
  pkgs.writeText "extras.conf" ''
    exec ${configure-gtk}/bin/configure-gtk
    exec ${configure-qt}/bin/configure-qt
