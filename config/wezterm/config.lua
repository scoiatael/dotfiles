...
local keys = {
	...
	{ key = "t", mods = cmd, action = act.SpawnCommandInNewTab({ cwd = wezterm.home_dir }) },
	{
		...
		action = act.SpawnCommandInNewTab({
			...
			args = { wezterm.home_dir .. "/.nix-profile/bin/zsh", "-c", "~/dotfiles/bin/__fzf_z.sh" },
		}),
	},
...
config.scrollback_lines = 1024 * 1024
config.adjust_window_size_when_changing_font_size = false
-- doesn't play nicely with INTEGRATED_BUTTONS
config.hide_tab_bar_if_only_one_tab = false
config.keys = keys
config.leader = leader
config.window_decorations = "INTEGRATED_BUTTONS|RESIZE"
config.window_background_opacity = 0.9
-- needed for modal.wezterm
